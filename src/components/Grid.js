    ...
    function handleClick(event) {
        ...
        if (grid.current[rowId][colId] === props.selectedOption) {
            if (grid.current[rowId][colId] === 'S') {
                setStart((prev) => { return { isExist: false, rowId: null, colId: null } });
            } else if (grid.current[rowId][colId] === 'E') {
                setEnd((prev) => { return { isExist: false, rowId: null, colId: null } });
            }
            ...
            setStart((prev) => { return { isExist: true, rowId: rowId, colId: colId } });
            grid.current[rowId][colId] = 'S';
        } else if (props.selectedOption === 'E') {
            if (grid.current[rowId][colId] === 'S') {
                setStart((prev) => { return { isExist: false, rowId: null, colId: null } });
            }
            setEnd((prev) => { return { isExist: true, rowId: rowId, colId: colId } });
        ...
        <div id="rows_parent">
            {
                grid.current.map((row, rowId) => {
                    return <div className="row" id={"row_" + rowId} key={rowId}>
                        {
                            row.map((cell, cellId) => {
                                return <Cell selectedOption={props.selectedOption} start={start['isExist']} end={end['isExist']} rowId={rowId} cellId={cellId} key={cellId} onClick={handleClick}></Cell>
                            }
                            )
                        }
                    </div>
